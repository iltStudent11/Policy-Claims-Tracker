...
echo "[1/4] Ensuring dedicated dev Mongo is running on host port 27018..."
ensure_container_running "pct-dev-mongo" "mongo:7" -p 27018:27017

echo "[2/4] Starting dev API on :5000 (Mongo: 127.0.0.1:27018)..."
...
echo "[3/4] Starting dev frontend on :5173..."
start_background_npm "$CLIENT_DIR" "$DEV_CLIENT_PID_FILE" "$DEV_CLIENT_LOG" npm run dev -- --host 127.0.0.1 --port 5173

echo "[4/4] Starting prod compose stack (client 8080/8443 + internal api/mongo)..."
cd "$ROOT_DIR"
docker compose -f docker-compose.prod.yml up -d --build >/dev/null

echo ""
echo "Dev + Prod are running:"
echo "- Dev frontend: http://localhost:5173"
echo "- Dev API:      http://localhost:5000"
echo "- Dev Mongo:    mongodb://127.0.0.1:27018"
echo "- Prod client:  http://localhost:8080 and https://localhost:8443"
echo ""
echo "Logs:"
echo "- Dev API log:     $DEV_API_LOG"
echo "- Dev client log:  $DEV_CLIENT_LOG"
